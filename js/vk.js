...
var PostsGet = async function () {
  ...
  if (bdrespones !== "bd_PostsRewrite_ok") {
    console.log("ERROR3: bd.PostsRewrite");
  }
  await pl.parsloop(1);
  var end = new Date();
...
var UsersGet = async function () {
  ...
  try {
    var {
      vkr
    } = await vkseq.call("groups.getMembers", {
      group_id: ownerid,
      offset: 0,
      count: 1000,
      fields: "first_name"
  ...
  if (vkr1.count > 1000) {
    ...
    for (let i = 0; i < vkr1.items.length; i++) {
      users.push({
        uid: vkr1.items[i].id,
        name: vkr1.items[i].first_name + " " + vkr1.items[i].last_name,
        balls: 0
      });
    }
    ...
    for (let maini = 1; maini != mainloop; maini++) {
      ...
      for (let i = 0; i < vkr2.items.length; i++) {
        users.push({
          uid: vkr2.items[i].id,
          name: vkr2.items[i].first_name + " " + vkr2.items[i].last_name,
          balls: 0
        });
      }
    }
  ...
  if (shopballsAdd.length !== 0) {
    ...
    if (bdrespones !== "bd_ShopBallsAdd_ok") {
      console.log("ERROR6: bd.ShopBallsAdd");
    }
  }

  var bdrespones = await bd.UsersRewrite(users);
  if (bdrespones !== "bd_UsersRewrite_ok") {
    console.log("ERROR7: bd.UsersRewrite");
  }
  await pl.parsloop(1);
  var end = new Date();
...
var PostLikesGetThread = async function (itemid, type) {
  ...
  try {
    var {
      vkr
    } = await vkseq.call("likes.getList", {
      item_id: itemid,
      owner_id: "-" + ownerid,
      filter: "likes",
      count: 1000,
...
var CommentsGetThread = async function (itemid) {
  ...
  try {
    var {
      vkr
    } = await vkseq.call("wall.getComments", {
      post_id: itemid,
      owner_id: "-" + ownerid,
      count: 100,
      need_likes: 1,
  ...
  if (vkr1.current_level_count > 100) {
    ...
    for (let maini = 1; maini < mainloop; maini++) {
      ...
      vkr1 = vkr;
      if (vkr1.items.length > 100) {
        commentloop = 100;
      } else {
        commentloop = vkr1.items.length;
      };
  ...
  async function Commentspars() {
    for (let i = 0; i < commentloop; i++) {
      if (!vkr1.items[i].deleted) {
        goodThread.push(vkr1.items[i].from_id)
      };
      ...
      if (vkr1.items[i].thread.count > 10) {
        ...
        for (let i2 = 0; i2 < thloop; i2++) {
          ...
          for (let i3 = 0; i3 < vkr2.items.length; i3++) {
            if (!vkr2.items[i3].deleted) {
              goodThread.push(vkr2.items[i3].from_id)
            };
        ...
        for (let i4 = 0; i4 < vkr1.items[i].thread.count; i4++) {
          ...
          if (!vkr1.items[i].thread.items[i4].deleted) {
            goodThread.push(vkr1.items[i].thread.items[i4].from_id);
          };
...
var UserAdd = async function (id) {
  ...
  var users = [{
    uid: Number(id),
    name: vkr1[0].first_name + " " + vkr1[0].last_name,
    balls: 0
  }];
  var shopballs = await bd.SBallsGet("All");
  var shopballsAdd = await c.comparison(users, shopballs);
  ...
  if (shopballsAdd.length !== 0) {
    var bdrespones = await bd.SBallsAdd(shopballsAdd);
    if (bdrespones !== "bd_SBallsAdd_ok") {
      console.log("ERROR14: bd.ShopBallsAdd");
    }
  }

  var bdrespones = await bd.UserAdd(users);
  if (bdrespones !== "bd_UserAdd_ok") {
    console.log("ERROR15: bd.UserAdd");
  }
  await pl.parsloop(1);
...
var WidgetUpdate = async function (uids) {
  ...
  try {
    var {
      vkr
    } = await vkgroup.call("appWidgets.update", {
      type: "tiles",
      code: 'return{\
    ...
    console.log("ERROR13 WidgetUpdate: " + err);
    await sleep(10000);
    WidgetUpdate(uids);
    return 0;
  }
...
var senditem = async function (id, cost, ostatok, item) {
  ...
  try {
    var {
      vkr
    } = await vkgroup.call('messages.send', {
      peer_id: id,
      message: mess,
      random_id: random
    });
